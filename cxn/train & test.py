...
for n, id_ in tqdm(enumerate(tr), total=350):
    ...
    mask = imread('/content/drive/My Drive/Colab Notebooks/dataset/BQA/train/' + str(int(id_)) + '.png')[:, :,
           :IMG_CHANNELS]
...
for n, id_ in tqdm(enumerate(te), total=10):
    ...
    mask = imread('/content/drive/My Drive/Colab Notebooks/dataset/BQA/test/' + str(int(170 + id_)) + '.png')[:, :,
           :IMG_CHANNELS]
    mask1 = resize(mask, (IMG_HEIGHT, IMG_WIDTH, 1), mode='constant', preserve_range=True)
    # mask1 = np.asarray(mask)
    # mask1 = resize(mask1, (IMG_HEIGHT, IMG_WIDTH, 1), mode='constant', preserve_range=True)
...
results = model.fit(X_train, Y_train, validation_split=0.05, batch_size=2, epochs=70,
