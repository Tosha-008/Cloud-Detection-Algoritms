...
class CommonBandsDataloader(Dataloader):
    ...
    def _read(self,path):
        ...
        if metadata['spacecraft_id']=='Sentinel2':
            ...
            ValueError('metadata of data doees not contain recognised satellite for SharedBandsDataloader')
...
class SEnSeITrainer(Sequence):
    ...
    def __getitem__(self,idx):
        ...
        if self.test_mode:
            ...
            # print(f"band_values shape: {band_values.shape}")
            # print(f"Possible descriptors shape: {possible_descriptors.shape}")
